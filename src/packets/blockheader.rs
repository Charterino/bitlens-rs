...
pub struct BlockHeader<'a> {
    pub version: u32,
    pub parent: Supercow<'a, [u8; 32]>,
    pub merkle_root: Supercow<'a, [u8; 32]>,
    pub timestamp: u32,
    pub bits: u32,
    pub nonce: u32,
...
impl BlockHeader<'_> {
    pub fn construct(
        ...
        let mut s = Self {
            version,
            parent: Supercow::owned(parent),
            merkle_root: Supercow::owned(merkle_root),
            timestamp,
            bits,
            nonce,
            txs_count,
...
impl<'a> Serializable<'a> for BlockHeader<'a> {
    fn deserialize(
        ...
        let parent = Supercow::borrowed(buffer.get_hash(4)?);
        let merkle_root = Supercow::borrowed(buffer.get_hash(36)?);
        ...
        match allocator.try_alloc(Self {
            ...
            Ok(result) => {
                if result.is_valid() {
                    Ok((Supercow::borrowed(result), 80 + offset))
                } else {
                    bail!("invalid header")
                }
            }
